...
class JpegImagesToMovie implements ControllerListener, DataSinkListener {
    ...
    public boolean doIt(int width, int height, int frameRate,
        ...
        try {
            ...
            System.err.println("[X]No se pudo crear el procesador de esa fuente "
                    + "de datos.");
            return false;
        }finally{
            System.err.println("[!]Procesador de imagenes creado correctamente");
        }
        ...
        if (!waitForState(p, Processor.Configured)) {
            System.err.println("[X]No se pudo configurar el procesador");
            return false;
        }
        ...
        if (f == null || f.length <= 0) {
            System.err.println("No se soporta el formato introducido: "
                    + tcs[0].getFormat());
            return false;
        }
